...
public class 拾起K个1需要的最少行动次数 {
    public static void main(String[] args) {
        ...
        System.out.println(minimumMoves(nums, 3, 1));
    }


    public static long minimumMoves(int[] nums, int k, int maxChanges) {
        List<Integer>pos=new ArrayList<>();
        int c=0;//数组中连续1的长度，如果超过k，就默认为k，因为就够用了
        for (int i = 0; i < nums.length; i++) {
            if(nums[i]==0)continue;
            pos.add(i);//记录1的位置
            c=Math.max(c,1);//如果c为0，c=c+1
            if(i>0&&nums[i-1]==1){
                if(i>1&&nums[i-2]==1){
                    c=3;//有3个连续的1
                }else {
                    c=Math.max(c,2);//如果c=1，则c=c+1
                }
            }
        }
        c=Math.min(c,k);//如果k很小，只需要k个连续的1就够用了
        if(maxChanges>=k-c){
            return Math.max(c-1,0)+2*(k-c);//防止没有1，时候返回0而不是-1
        }

        int n=pos.size();
        long[]sum=new long[n+1];
        for (int i = 0; i < n; i++) {
            sum[i+1]=sum[i]+pos.get(i);//前缀和数组，'1'距离的前缀和
        }
        ...
        for(int right=size;right<=n;right++){
            ...
            int left=right-size;
            int i=left+size/2;
            long index=pos.get(i);//中位数的索引
            long s1=index*(i-left)-(sum[i]-sum[left]);//要减去一半1的数量个index，就是左边距离中位数距离之和
